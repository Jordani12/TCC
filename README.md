# TCC
 ...
 ---------------------------------------------------------------------------
  ...
  ESC-F                Like F but stop when search pattern is found.
 ESCOC
s the Ctrl key; thus ^K is ctrl-K.
...
ow (or N lines).
ow (or N lines).
ow (and set window to N).
ow (and set window to N).
ow, but don't stop at end-of-file.
-window (and set half-window to N).
-window (and set half-window to N).
reen width (or N positions).
reen width (or N positions).
umn displayed.
lumn.
like "tail -f".
hen search pattern is found.
 ESCOD
      A key preceded by a caret indicates the Ctrl key; thus ^K is ctrl-K.
